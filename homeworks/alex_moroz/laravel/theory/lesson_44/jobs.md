...
Во время выполнения запроса, есть скрипт, который м.б. выполнен вне 
запроса пользователя - эту часть логики выносят в отдельный скрипт - джобу. 
Для его выполнения используются механизмы очередей и worker-ов, при выполнении скрипта, 
джоба помещается в очередь на выполнение, а запрос продолжает работать дальше. 
Запрос - помещает джобу в очередь. 

Воркер - запущенный на сервере скрипт, который постоянно выполняется и следит за состоянием очереди: 
при появлении новых джоб, выполняет их. FIFO.
