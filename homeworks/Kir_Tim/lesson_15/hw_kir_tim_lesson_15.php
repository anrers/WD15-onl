...
{
    $idInArray = array_column($products, 'id');

    if (in_array($id, $idInArray)) {
        return;
    } else {
        $products[] = ['id' => $id,
            'name' => $name,
...
function getValueByKey($key, $array)
{
    if (array_key_exists($key, $array)) {
        echo "$key имеет стоимость $array[$key]" . PHP_EOL;
    }
    return null;
}

getValueByKey('banana', ["lemon" => 1, "orange" => 3, "banana" => 5, "apple" => 12]) . PHP_EOL;
...
doubleArrayValues($arrayForChanges, $callDoubleValue) . PHP_EOL;
